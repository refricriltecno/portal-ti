...
const LISTA_STATUS = ["Pendente envio do boleto", "Pendente envio da Nota", "Enviado nota e boleto", "Pago"];
...
function App() {
  ...
  const [user, setUser] = useState({ username: '', role: '', foto_perfil: null });
  ...
  const [newUserForm, setNewUserForm] = useState({ username: '', password: '', role: 'normal' });
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('token', data.access_token); setToken(data.access_token); setUser({ username: data.username, role: data.role, foto_perfil: data.foto_perfil });
      setMsg({tipo: 'success', texto: 'Bem-vindo!'});
    } catch { setMsg({tipo: 'error', texto: 'Usuário ou senha incorretos.'}); }
    setLoading(false);
  };

  const logout = () => { localStorage.removeItem('token'); setToken(null); setUser({ username: '', role: '', foto_perfil: null }); };
  ...
  const handleUploadFotoPerfil = async (e) => {
    e.preventDefault(); 
    if (!fotoPerfilFile) { setMsg({tipo: 'error', texto: 'Selecione uma imagem'}); return; }
    setLoading(true);
    const fd = new FormData();
    fd.append('foto', fotoPerfilFile);
    try {
      const res = await fetch(`${API_URL}/me/foto`, { method: 'POST', headers: authHeader, body: fd });
      if(!res.ok) {
        const err = await res.json();
        throw new Error(err.detail || 'Erro ao fazer upload');
      }
      const data = await res.json();
      setMsg({tipo: 'success', texto: 'Foto de perfil atualizada com sucesso!'});
      setFotoPerfilFile(null);
      setUser({...user, foto_perfil: data.foto_url});
    } catch (err) { setMsg({tipo: 'error', texto: err.message || 'Erro ao fazer upload'}); }
    setLoading(false);
  };
  ...
  useEffect(() => { 
    if(token) {
      try { 
        ...
        // Carregar dados do usuário do backend (/me) para ter foto_perfil atualizada
        const carregarUsuario = async () => {
          try {
            const res = await fetch(`${API_URL}/me`, {headers: authHeader});
            if(res.ok) {
              const data = await res.json();
              setUser({ username: data.username, role: data.role, foto_perfil: data.foto_perfil });
              // Salvar foto no localStorage para persistência
              if(data.foto_perfil) localStorage.setItem('userFotoPerfil', data.foto_perfil);
            }
        ...
        const interval = setInterval(() => {
          carregarDados();
          carregarUsuario(); // Atualizar foto periodicamente
  ...
  const carregarLogs = async () => { 
    const endpoint = user.role === 'admin' ? `${API_URL}/logs/` : `${API_URL}/historico/`;
  ...
  const criarNovoUsuario = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/users/`, { method: 'POST', body: fd, headers: authHeader });
      ...
      setNewUserForm({ username: '', password: '', role: 'normal' });
      carregarUsuarios();
    } catch (err) { setMsg({tipo: 'error', texto: err.message || 'Erro ao criar usuário'}); }
    setLoading(false);
  };

  const atualizarRoleUsuario = async (usuarioId, novoRole) => {
    setLoading(true);
    try {
      const fd = new FormData();
      fd.append('role', novoRole);
      const res = await fetch(`${API_URL}/users/${usuarioId}/role`, { 
        method: 'PUT', 
        body: fd, 
        headers: authHeader 
      });
      if(!res.ok) throw new Error('Erro ao atualizar perfil');
      setMsg({tipo: 'success', texto: 'Perfil atualizado com sucesso!'});
  ...
  const inativarContrato = async (id) => { 
    if(confirm("Inativar este contrato? Ele deixará de aparecer na lista mas pode ser reativado depois.")) { 
      try { 
        const res = await fetch(`${API_URL}/contratos/${id}/inativar`, {method:'PUT', headers: authHeader}); 
        if(res.ok) { setMsg({tipo: 'success', texto: 'Contrato inativado com sucesso'}); carregarDados(); } 
        else setMsg({tipo: 'error', texto: 'Erro ao inativar'}); 
      } catch(err) { setMsg({tipo: 'error', texto: 'Erro ao inativar contrato'}); } 
    } 
  };
  ...
  if (!token) return (
    <div className="flex h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-gradient-to-br from-slate-900 to-slate-950 p-8 rounded-2xl border border-slate-800/50 shadow-2xl">
          <div className="text-center mb-8">
            <div className="mx-auto mb-4 flex justify-center">
              <img src={LogoRefricril} alt="Refricril" className="h-20 object-contain drop-shadow-lg" />
            </div>
            <h1 className="text-3xl font-black bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Sistema de Gestão</h1>
            <p className="text-sm text-slate-500 mt-2">Contratos e Telefonia</p>
          </div>
        ...
        <p className="text-center text-xs text-slate-600 mt-6">© 2026 - Sistema de Gestão</p>
      </div>
    </div>
  );

  const inativarFatura = async (id) => { 
    if(confirm("Inativar esta fatura? Ela deixará de aparecer na lista mas pode ser reativada depois.")) { 
      try { 
        const res = await fetch(`${API_URL}/faturas/${id}/inativar`, {method:'PUT', headers: authHeader}); 
        if(res.ok) { setMsg({tipo: 'success', texto: 'Fatura inativada com sucesso'}); carregarDados(); } 
        else setMsg({tipo: 'error', texto: 'Erro ao inativar'}); 
      } catch(err) { setMsg({tipo: 'error', texto: 'Erro ao inativar fatura'}); } 
    } 
  };
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-white font-sans selection:bg-cyan-500/30">
      <div className={`${sidebarOpen ? 'w-72' : 'w-20'} bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border-r border-slate-800/50 transition-all duration-300 flex flex-col z-20 shadow-2xl`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800/50"><img src={LogoRefricril} alt="Refricril" className="h-10 object-contain" />{sidebarOpen && <div><h1 className="font-black text-lg bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">Sistema de Gestão</h1><p className="text-xs text-slate-400">v2.5 Pro</p></div>}</div>
        <nav className="flex-1 px-4 space-y-2 mt-6">
          ...
          <button onClick={()=>{setActivePage('historico'); carregarLogs();}} className={`w-full flex gap-3 px-4 py-3 rounded-lg transition-all ${activePage==='historico'?'bg-amber-500/10 text-amber-400 border border-amber-500/30':'text-slate-400 hover:bg-slate-800/50'}`}><History size={20}/> {sidebarOpen && <span className="font-semibold">Histórico</span>}</button>
          
          {/* Credenciais disponível para admin e tercerizado */}
          {(user.role === 'admin' || user.role === 'tercerizado') && <button onClick={()=>{setActivePage('credenciais'); carregarCredenciais();}} className={`w-full flex gap-3 px-4 py-3 rounded-lg transition-all ${activePage==='credenciais'?'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30':'text-slate-400 hover:bg-slate-800/50'}`}><Lock size={20}/> {sidebarOpen && <span className="font-semibold">Credenciais</span>}</button>}
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="h-20 border-b border-slate-800/30 flex items-center px-8 justify-between backdrop-blur-md bg-gradient-to-r from-slate-950/80 via-slate-950/80 to-slate-900/80 z-10 shadow-xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-slate-800 cursor-pointer hover:opacity-80 transition-all" onClick={()=>setModalProfile(true)}>
              ...
              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center shadow-lg shadow-cyan-500/30 overflow-hidden">
                {user.foto_perfil ? <img src={`${API_URL}/${user.foto_perfil}`} alt="Perfil" className="w-full h-full object-cover" /> : <User size={18} className="text-white"/>}
              </div>
            </div>
          </div>
        </header>
        <main className="flex-1 overflow-auto p-8 scrollbar-thin scrollbar-thumb-slate-800">
          ...
          {activePage === 'historico' && <div className="space-y-6"><h1 className="text-3xl font-bold">Histórico</h1><div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden"><table className="w-full text-left text-sm text-slate-300"><thead className="bg-slate-900/50 text-slate-400"><tr><th className="p-4">Data</th><th className="p-4">Usuário</th><th className="p-4">Detalhes</th></tr></thead><tbody>{logs.map(l=><tr key={l.id} className="hover:bg-slate-700/30"><td className="p-4">{new Date(l.data_hora).toLocaleString()}</td><td className="p-4 font-bold text-white">{l.usuario}</td><td className="p-4">{l.acao} - {l.detalhes}</td></tr>)}</tbody></table></div></div>}
          {activePage === 'credenciais' && (user.role === 'admin' || user.role === 'tercerizado') && <div className="space-y-6 animate-in fade-in"><div className="flex justify-between items-center"><h1 className="text-3xl font-bold">Credenciais</h1>{user.role === 'admin' && <button onClick={abrirModalNovaCredencial} className="bg-emerald-600 px-6 py-2 rounded-xl font-bold flex gap-2"><Plus/> Nova</button>}</div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{credenciais.map(c=><div key={c.id} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 hover:border-slate-600 transition-all group"><div className="flex justify-between items-start mb-3"><div className="flex-1"><h3 className="font-bold text-cyan-400 text-lg">{c.nome_servico}</h3><p className="text-xs text-slate-400 mt-1">{c.descricao}</p></div>{user.role === 'admin' && <div className="flex gap-2"><button onClick={()=>abrirModalEditarCredencial(c)} className="text-slate-500 hover:text-blue-400 transition-colors"><Edit size={16}/></button><button onClick={()=>excluirCredencial(c.id)} className="text-slate-500 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button></div>}</div><div className="space-y-2 text-sm"><div className="flex items-center gap-2"><span className="text-slate-500 w-16">URL:</span><a href={c.url_acesso} target="_blank" rel="noopener" className="text-cyan-400 hover:underline truncate">{c.url_acesso}</a></div><div className="flex items-center gap-2"><span className="text-slate-500 w-16">User:</span><span className="text-slate-300 font-mono">{c.usuario}</span><button className="text-slate-500 hover:text-slate-300" onClick={()=>navigator.clipboard.writeText(c.usuario)}><Copy size={14}/></button></div><div className="flex items-center gap-2"><span className="text-slate-500 w-16">Pass:</span><span className="text-slate-400 select-none">{'•'.repeat(8)}</span><button className="text-slate-500 hover:text-slate-300" onClick={()=>navigator.clipboard.writeText(c.senha)} title="Copiar senha"><Copy size={14}/></button></div>{c.email && <div className="flex items-center gap-2"><span className="text-slate-500 w-16">Email:</span><span className="text-slate-300">{c.email}</span></div>}{c.telefone && <div className="flex items-center gap-2"><span className="text-slate-500 w-16">Tel:</span><span className="text-slate-300">{c.telefone}</span></div>}{c.responsavel && <div className="flex items-center gap-2"><span className="text-slate-500 w-16">Resp:</span><span className="text-slate-300">{c.responsavel}</span></div>}</div><div className="mt-3 pt-3 border-t border-slate-700/50"><span className="text-[10px] text-slate-500">Atualizado: {new Date(c.data_atualizacao).toLocaleDateString('pt-BR')}</span></div></div>)}</div></div>}
          {activePage === 'admin' && user.role === 'admin' && <div className="space-y-6 animate-in fade-in"><h1 className="text-3xl font-bold">Gestão de Usuários</h1><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-emerald-500/30"><h3 className="font-bold text-emerald-400 mb-4 flex items-center gap-2"><Plus size={18}/> Novo Usuário</h3><form onSubmit={criarNovoUsuario} className="space-y-4"><div><label className="text-xs font-bold text-slate-300 mb-1 block">Usuário (nome.sobrenome)</label><input required placeholder="ex: joao.silva" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white" value={newUserForm.username} onChange={e=>setNewUserForm({...newUserForm, username:e.target.value})}/></div><div><label className="text-xs font-bold text-slate-300 mb-1 block">Senha</label><input required type="password" placeholder="Mínimo 6 caracteres" className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white" value={newUserForm.password} onChange={e=>setNewUserForm({...newUserForm, password:e.target.value})}/></div><div><label className="text-xs font-bold text-slate-300 mb-1 block">Perfil/Grupo</label><select className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white" value={newUserForm.role} onChange={e=>setNewUserForm({...newUserForm, role:e.target.value})}><option value="normal">👤 Normal - Dashboard, Contratos, Faturas, Telefonia</option><option value="admin">⚙️ Admin - Acesso Total</option><option value="tercerizado">🤝 Tercerizado - Normal + Credenciais</option></select></div><button type="submit" disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-500 py-2 rounded-lg font-bold text-white transition-all disabled:opacity-50">{loading ? 'Criando...' : 'Criar Usuário'}</button></form></div><div className="lg:col-span-2 bg-slate-800/50 p-6 rounded-2xl border border-slate-700/50"><h3 className="font-bold text-cyan-400 mb-4 flex items-center gap-2"><User size={18}/> Usuários Cadastrados</h3><div className="space-y-2 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700">{usersList.length === 0 ? <p className="text-slate-500 text-center py-4">Nenhum usuário cadastrado</p> : usersList.map(u=><div key={u.id} className="flex justify-between items-center bg-slate-900/50 p-3 rounded-lg border border-slate-700 hover:border-slate-600 transition-all"><div className="flex-1"><p className="font-mono text-white text-sm">{u.username}</p><p className="text-xs text-slate-500">{u.is_active ? '✓ Ativo' : '✗ Inativo'}</p></div>{editandoUsuario === u.id ? <div className="flex items-center gap-2"><select value={u.role} onChange={e=>atualizarRoleUsuario(u.id, e.target.value)} className="text-xs bg-slate-800 border border-slate-600 rounded px-2 py-1 text-white"><option value="normal">Normal</option><option value="admin">Admin</option><option value="tercerizado">Tercerizado</option></select><button onClick={()=>setEditandoUsuario(null)} className="text-slate-400 hover:text-blue-400 p-1"><CheckSquare size={14}/></button></div> : <div className="flex items-center gap-3"><span className={`text-xs px-2 py-1 rounded ${u.role === 'admin' ? 'bg-amber-500/20 text-amber-400 font-bold' : u.role === 'tercerizado' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-blue-500/20 text-blue-400'}`}>{u.role === 'admin' ? '⚙️ Admin' : u.role === 'tercerizado' ? '🤝 Tercerizado' : '👤 Normal'}</span><button onClick={()=>setEditandoUsuario(u.id)} className="text-slate-500 hover:text-yellow-400 transition-colors p-1" title="Editar perfil"><Edit size={14}/></button><button onClick={()=>deletarUsuario(u.id)} className="text-slate-500 hover:text-rose-500 transition-colors p-1" title="Deletar"><Trash2 size={14}/></button></div>}</div>)}</div></div></div></div>}
        </main>
      </div>
      ...
      <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/30"><h3 className="text-sm font-bold text-orange-400 uppercase mb-4">Documento do Contrato</h3>
      <div className="border-2 border-dashed border-orange-500/30 hover:border-orange-500/60 rounded-xl p-6 text-center cursor-pointer relative transition-all bg-orange-500/5"><input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt" className="absolute inset-0 opacity-0 cursor-pointer" onChange={e=>setFormContrato({...formContrato, arquivo:e.target.files?.[0]||null})}/><Upload size={32} className="mx-auto text-orange-400/50 mb-2 transition-colors" /><p className="text-sm font-semibold text-slate-300">{formContrato.arquivo ? <span className="text-orange-400">{formContrato.arquivo.name}</span> : "Clique ou arraste para selecionar"}</p><p className="text-xs text-slate-500 mt-1">PDF, DOC, XLSX ou TXT (Opcional)</p></div></div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 rounded-2xl w-full max-w-md border border-slate-700 shadow-2xl">
            <div className="p-6 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
              <h2 className="text-2xl font-bold text-white flex items-center gap-2"><User size={24} className="text-cyan-400"/> Meu Perfil</h2>
              <button onClick={()=>setModalProfile(false)}><X className="text-slate-500 hover:text-white"/></button>
            </div>
            <div className="p-6 space-y-6">
              {/* Seção de Foto de Perfil */}
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                <p className="text-xs text-slate-400 mb-3 font-bold uppercase">Foto de Perfil</p>
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center overflow-hidden">
                    {user.foto_perfil ? <img src={`${API_URL}/${user.foto_perfil}`} alt="Perfil" className="w-full h-full object-cover" /> : <User size={32} className="text-white"/>}
                  </div>
                  <div className="flex-1">
                    <label className="block text-xs font-bold text-slate-300 mb-2">Selecionar Imagem</label>
                    <input type="file" accept="image/*" onChange={e=>setFotoPerfilFile(e.target.files?.[0]||null)} className="text-xs text-slate-400 file:bg-cyan-600 file:text-white file:border-0 file:px-3 file:py-1 file:rounded cursor-pointer"/>
                    {fotoPerfilFile && <button onClick={handleUploadFotoPerfil} disabled={loading} className="mt-2 w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 rounded-lg transition-all disabled:opacity-50 text-sm">{loading ? 'Upload...' : 'Enviar Foto'}</button>}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                ...
                <p className="text-xs text-slate-500 mt-2 capitalize">Role: {user.role}</p>
              </div>

              {/* Form de Alterar Senha */}
              <form onSubmit={handleChangePassword} className="space-y-4">
                <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                  <p className="text-xs text-slate-400 mb-3 font-bold uppercase">Alterar Senha</p>
                  <div className="space-y-3">
                    <div>
                      <label className="text-xs font-bold text-slate-300 mb-2 block">Senha Atual</label>
                      <input required type="password" placeholder="Digite sua senha atual" className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 text-white placeholder-slate-500 transition-all focus:outline-none focus:border-slate-600" value={passwordForm.senhaAtual} onChange={e=>setPasswordForm({...passwordForm, senhaAtual:e.target.value})}/>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-300 mb-2 block">Nova Senha</label>
                      <input required type="password" placeholder="Mínimo 6 caracteres" className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 text-white placeholder-slate-500 transition-all focus:outline-none focus:border-slate-600" value={passwordForm.senhaNova} onChange={e=>setPasswordForm({...passwordForm, senhaNova:e.target.value})}/>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-300 mb-2 block">Confirmar Nova Senha</label>
                      <input required type="password" placeholder="Repita a nova senha" className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 text-white placeholder-slate-500 transition-all focus:outline-none focus:border-slate-600" value={passwordForm.senhaConfirm} onChange={e=>setPasswordForm({...passwordForm, senhaConfirm:e.target.value})}/>
                    </div>
                  </div>
                  <div className="flex gap-3 pt-4 border-t border-slate-700">
                    <button type="button" onClick={()=>setModalProfile(false)} className="flex-1 px-4 py-2 text-slate-400 hover:bg-slate-800 rounded-lg transition-all">Cancelar</button>
                    <button type="submit" disabled={loading} className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 rounded-lg transition-all disabled:opacity-50">{loading ? 'Alterando...' : 'Alterar Senha'}</button>
                  </div>
                </div>
              </form>
            </div>
